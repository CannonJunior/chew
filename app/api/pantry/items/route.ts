...
export async function GET(req: NextRequest) {
  ...
  const rows =
    conditions.length > 0
      ? db.select().from(groceryItems).where(and(...conditions)).orderBy(desc(groceryItems.createdAt)).all()
      : db.select().from(groceryItems).orderBy(desc(groceryItems.createdAt)).all();
