 ...
 *   2. runPrompt subprocess reads the image → returns JSON items
 *   3. POST items to Chew /api/pantry/receipts/:id/items
 */
import fs from 'node:fs';
import path from 'node:path';
export async function processReceiptImage(imagePath, chewUrl, runPrompt) {
    ...
    const result = await runPrompt(parsePrompt);
    if (!result.success) {
    ...
    const lines = [`Pantry updated — ${items.length} items saved to Chew.`];
